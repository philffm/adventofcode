...
hgt:183cm cid:187 byr:2019 ecl:xry iyr:2013 pid:164cm hcl:#18171d eyr:2021`;
    ...
    // => passport.trim().split(/\s+/)); RexEx to exclude all empty spaces
var currentPass;
var passCount;
...
function validatePassportRequiredFields(input) {
    const passports = input.split("\n\n"); // split new line
    const requiredFields = ['byr:','iyr:','eyr:','hgt:','hcl:','ecl:','pid:'];
    let validPassportCount = 0;
    
    for (let passport of passports) {
        let passportIsValid = 1;
        for (let field of requiredFields) {
            if (!passport.includes(field)) {
                passportIsValid = 0;
            }
        }
        validPassportCount += passportIsValid;
    }
...
function validateAllPassports(input) {
    const passports = input.split("\n\n");
    let validPassportCount = 0;

    for (let passport of passports) {
        validPassportCount += validatePassport(passport);
    }
...
function validatePassport(passport) {
    //I attempted to do most of this validation using regex but JS was being a turd and wanted me to
    //re-execute the regex for every single named group. This also means I'd have to loop through the
    //results of each execution, manually assembling the results into a single object. I went with this
    // approach instead.
    passport = passport.replaceAll('\n', ' ');
    passport = passport.split(' ');
    const requiredFieldLength = 7;
    
    if (passport.length < requiredFieldLength) {
        return 0;
    }

    let passportObject = {};
    for (let keyValue of passport) {
        keyValue = keyValue.split(':');
        passportObject[keyValue[0]] = keyValue[1]; 
    }

    if (!validatePassportHasRequiredFields(passportObject)) {
        return 0;
    }
    
    if (!validateYear(passportObject.byr, 1920, 2002)) {
        return 0;
    }

    if (!validateYear(passportObject.iyr, 2010, 2020)) {
        return 0;
    }
    
    if (!validateYear(passportObject.eyr, 2020, 2030)) {
        return 0;
    }
    
    if (!validateHeight(passportObject.hgt)) {
        return 0;
    }

    if (!validateHairColor(passportObject.hcl)) {
        return 0;
    }
    
    if (!validateEyeColor(passportObject.ecl)) {
        return 0;
    }
    
    if (!validatePassportId(passportObject.pid)) {
        return 0;
    }
...
function validatePassportHasRequiredFields(passport) {
...
function validateYear(inputYear, earliest, latest) {
    if (String(inputYear).length !== 4) {
        return false;
    }
    inputYear = Number(inputYear);
    return inputYear >= earliest && inputYear <= latest;
}
...
function validateHeight(height) {
    ...
    let number = Number(height.replace(units, ''));

    if (number < limits[units]['lower'] || number > limits[units]['upper'])
    {
        return false;
    }
...
validateAllPassports(input);





// function splitIntoSubArray(arr) {
//     var newArray = [];
//     let currentRange = 0;
...
//         if(arr[i]==""){
//             passportFragments.splice(i,1);
//             newArray.push(arr.splice(0,currentRange-1));
